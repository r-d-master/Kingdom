...
{
    global $lineWidth;
    #if (!isset($lineWidth)) {$lineWidth = 0;}
    ...
    foreach ($modules as $module_key => $module) {
        ...
        if ($module['name']=="bg_start" || $module['name']=="bg_end") {
            if ($module['name']=="bg_start") {
                echo '<div class="module_line_trigger" data-option="'.(($module['shadow']) == "yes" ? 'hasShadow' : '').'" data-background="'.((strlen($module['bg_color'])>0) ? "#".$module['bg_color'] : '').' '.((strlen($module['bg_image'])>0) ? "url('".$module['bg_image']."')" : '').' repeat 0 0" data-top-padding="'.$module['padding_top'].'" data-bottom-padding="'.$module['padding_bottom'].'">';
            }
            if ($module['name']=="bg_end") {
                if ($lineWidth<1) {
                    echo '</div>';
                }
                echo '</div><!-- .module_line_trigger -->';
                $lineWidth = 1;
            }
        }
        else {
            ...
            echo "<div class='{$outputClass} ".(($normalModulesICounter==1) ? "first-module" : "")." module_number_{$normalModulesICounter} module_cont {$module['padding_bottom']} module_{$module['name']}'>";
            ...
            switch ($module['name']) {
                ...
                case "content":
                    ...
                    if (strlen($module['heading_color'])>0) {$custom_color = "color:#{$module['heading_color']};";} else {$custom_color = "";}
                    if (strlen($module['heading_text'])>0) {
                        echo "<div class='bg_title'><".$module['heading_size']." style='".$custom_color."' class='headInModule'>{$module['heading_text']}</".$module['heading_size']."></div>";
                    }
