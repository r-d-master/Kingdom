...
function get_media_for_this_post($postid, $page="1")
{
    ...
    $args = array(
        ...
        'numberposts' => $pbconfig['images_from_media_library'],
        'post_status' => null,
        'order' => 'DESC',
        'paged' => $page
        /*'post_parent' => $postid*/
    ...
    if (is_array($images) && $images) {
        foreach ($images as $image) {
            ...
            if ((isset($meta['width']) && $meta['width'] > 0) && !isset($meta['audio'])) {
                $imgpack[] = array("guid" => $image->guid, "width" => $meta['width'], "height" => $meta['height']);
            }
        }
        return $imgpack;
    }
...
function get_selected_pf_images_for_admin($pagebuilder)
{
    if (!isset($compile)) {$compile='';}
    if (isset($pagebuilder['post-formats']['images']) && is_array($pagebuilder['post-formats']['images'])) {
        foreach ($pagebuilder['post-formats']['images'] as $imgid => $img) {
            $compile .= "<div class='img-item style_small'><div class='img-preview'><img src='".aq_resize($img['src'], "62", "62", true)."' data-full-url='{$img['src']}' data-timthumb-url='".aq_resize($img['src'], "156", "106", true)."' alt='' class='previmg'><div class='hover-container'></div><div class='deldel-container'></div></div><input type='hidden' name='pagebuilder[post-formats][images][" . $imgid . "][src]' value='{$img['src']}'></div>";
        }
    }
    return $compile;
}
...
function get_selected_pf_images($pagebuilder)
{
    if (is_array($pagebuilder['post-formats']['images'])) {
        if (!isset($compile)) {$compile='';}
        if (count($pagebuilder['post-formats']['images'])==1) {$onlyOneImage = "oneImage";} else {$onlyOneImage = "";}
        $compile .= '
        ...
        if (is_array($pagebuilder['post-formats']['images'])) {
            foreach ($pagebuilder['post-formats']['images'] as $imgid => $img) {
                $compile .= '
                    <img src="'.aq_resize($img['src'], "1170", "563", true).'" data-thumb="'.aq_resize($img['src'], "1170", "563", true).'" alt="" />
                ';
            }
        }
            ...
            if ($style == "small") {
                ...
                <div class='img-item style_small available_media_item'>
                    <div class='img-preview'>
                        <img class='previmg' alt='' data-timthumb-url='" . aq_resize($media_url, "156", "106", true) . "' data-full-url='" . $media_url . "' src='" . aq_resize($media_url, "62", "62", true) . "'>
            ...
            if ($slider_type == "fullscreen") {
                ...
                if ($slide['slide_type'] == "image") {
                    ...
                    <div class='img-item item-with-settings'>
                        ...
                        <div class='img-preview'>
                            <img alt='' src='" . aq_resize($slide['src'], "156", "106", true) . "'>
                            <div class='hover-container'>
...
function get_def_bg_pos_admin_options_style()
{
    ...
    if (is_array($pbconfig['page_bg_available_types'])) {
        foreach ($pbconfig['page_bg_available_types'] as $bg_position) {
...
function get_theme_pagebuilder($postid)
{
    ...
    if (!isset($pagebuilder['settings']['show_content_area'])) {$pagebuilder['settings']['show_content_area'] = "yes";}
    if (!isset($pagebuilder['settings']['show_page_title'])) {$pagebuilder['settings']['show_page_title'] = "yes";}
    if (!isset($pagebuilder['settings']['layout-sidebars'])) {$pagebuilder['settings']['layout-sidebars'] = get_theme_option("default_sidebar_layout");}
    if (!isset($pagebuilder['settings']['show_breadcrumb'])) {$pagebuilder['settings']['show_breadcrumb'] = get_theme_option("default_breadcrumb_status");}
...
function update_theme_pagebuilder($post_id, $variableName, $pagebuilderArray)
{
    array_walk_recursive($pagebuilderArray, 'replace_rn_to_br_in_multiarray');
...
{
    $pagebuilder['settings']['layout-sidebars'] = get_theme_option("default_sidebar_layout");
    $pagebuilder['settings']['left-sidebar'] = "Default";
    $pagebuilder['settings']['right-sidebar'] = "Default";
    $pagebuilder['settings']['bg_image']['status'] = get_theme_option("show_bg_img_by_default");
    $pagebuilder['settings']['bg_image']['src'] = get_theme_option("bg_img");
    $pagebuilder['settings']['custom_color']['status'] = get_theme_option("show_bg_color_by_default");
        ...
        $shortcodes_array = array(
            "custom_button" => array(
                ...
                "options" =>
                    ...
                    get_empty_select("Type", "custom_button_sc_type", $pbconfig['all_available_custom_buttons'], $width="250px").
                    get_empty_select("Target", "custom_button_target", $pbconfig['all_available_target_for_custom_buttons'], $width="250px").
            ...
            "blockquote" => array(
                ...
                "options" =>
                    ...
                    get_empty_select("Type", "quote_type", $pbconfig['all_available_quote_types'], $width="150px").
            ...
            "dropcaps" => array(
                ...
                "options" =>
                    ...
                    get_empty_select("Type", "dropcaps_sc_type", $pbconfig['all_available_dropcaps'], $width="150px").
            ...
            "highlighter" => array(
                ...
                "options" =>
                    ...
                    get_empty_select("Type", "highlighter_sc_type", $pbconfig['all_available_highlighters'], $width="150px").
            ...
            "divider" => array(
                ...
                "title" => "Divider",
                "options" =>
                    get_empty_select("Type", "dividers_sc_type", $pbconfig['all_available_dividers'], $width="200px").
            ...
            "social" => array(
                ...
                "options" =>
                    ...
                    get_empty_select("Type", "social_sc_type", $pbconfig['all_available_social_icons_type'], $width="150px").
                    get_empty_select("Style", "social_sc_style", $pbconfig['all_available_social_icons'], $width="150px").
...
function the_pb_custom_bg_and_color($pagebuilder) {
    $cover = ''; $bgcolor = ''; $bgimg = ''; $repeat = ''; $fullcolor = ''; $fullimg  = ''; $classimg  = '';
    if (!isset($pagebuilder['settings']['layouts']['type'])) {$pagebuilder['settings']['layouts']['type'] = "default";}
    if ($pagebuilder['settings']['layouts']['type'] == "default") {
        $page_layout = get_theme_option("default_theme_layout");
    } else {
        $page_layout = $pagebuilder['settings']['layouts']['type'];
    }

    /*
    We have:
    $page_layout - layout for current page
    */

    switch ($page_layout) {

        case "clean":
            if (isset($pagebuilder['settings']['bg_image']['status']) && $pagebuilder['settings']['bg_image']['status'] == "on")
            {
                $bgimg = $pagebuilder['settings']['bg_image']['src'];
            } else {
                $bgimg = get_theme_option("bg_pattern");
            }
            if (isset($pagebuilder['settings']['custom_color']['status']) && $pagebuilder['settings']['custom_color']['status'] == "on") {
                $bgcolor = $pagebuilder['settings']['custom_color']['value'];
            } else {
                $bgcolor = get_theme_option("default_bg_color");
            }
            #echo '<div class="layout_trigger clean_bg_cont" style="background-image:url('.$bgimg.'); background-repeat: repeat; background-color:#'.$bgcolor.';"></div>';
            echo '<div class="layout_trigger clean_bg_cont" style=""></div>';
            break;
        ...
        case "fullscreen_bg_image":
            ...
            echo '<div data-addon_pattern="'.get_theme_option("bg_pattern").'" data-addon_color="'.$bgcolor.'" class="layout_trigger image_bg_cont" style="background-image:url('.$bgimg.'); background-repeat: no-repeat; background-color:#'.$bgcolor.';"></div>';
            break;
    }
    ...
    /*if (isset($pagebuilder['settings']['bg_image']['status']) && $pagebuilder['settings']['bg_image']['status'] == "on") {
        $bgimg = $pagebuilder['settings']['bg_image']['src'];
        if ($pagebuilder['settings']['bg_image']['type'] == 'center') {
            $repeat = 'no-repeat';
            $cover = '';
        }
        if ($pagebuilder['settings']['bg_image']['type'] == 'repeat') {
            $repeat = 'repeat';
            $cover = '';
        }
        if ($pagebuilder['settings']['bg_image']['type'] == 'stretched') {
            $repeat = 'no-repeat';
            $cover = 'covered';
        }
    ...
    /* BG COLOR */
    /*if (isset($pagebuilder['settings']['custom_color']['status']) && $pagebuilder['settings']['custom_color']['status'] == "on") {
        $fullcolor = 'background-color:#'.$pagebuilder['settings']['custom_color']['value'].';';
    }*/

    #START ECHO
    /*if ((isset($pagebuilder['settings']['custom_color']['status']) && $pagebuilder['settings']['custom_color']['status'] == "on") || (isset($pagebuilder['settings']['bg_image']['status']) && $pagebuilder['settings']['bg_image']['status'] == "on"))
    {
        echo '<div class="custom_bg_cont '.$cover.' '.$classimg.'" style="'.$fullcolor.' '.$fullimg.'"></div>';
    } elseif ((isset($pagebuilder['settings']['custom_color']['status']) && $pagebuilder['settings']['custom_color']['status'] == "off") || (isset($pagebuilder['settings']['bg_image']['status']) && $pagebuilder['settings']['bg_image']['status'] == "off")) {
        /* nothing here */
    /*} else {
        echo '<div class="custom_bg_cont" style="background-color:#'.get_theme_option("default_bg_color").' '.$fullimg.'"></div>';
    }*/
}
