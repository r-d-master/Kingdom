...
function get_theme_option($optionname, $defaultValue="")
{
    global $themeshort;
...
function showPortCategory($postid = "")
{
    if (!isset($term_list)) {
        $term_list = '';
    }
...
function sendFeedback($feedback_email, $feedback_msg, $feedback_name, $subject)
{
	...
	<html>
	<head>
	  <title>Email from " . get_bloginfo('name') . "</title>
	</head>
	<body>
...
function teams_save_postdata($post_id)
{
    ...
    if (isset($_POST)) {
        ...
        if (isset($_POST['teams_position'])) {
            $teams_position = $_POST['teams_position'];
...
function the_breadcrumb() {
    $showOnHome = 1;
    $delimiter = '';
    $home = ((get_theme_option("translator_status") == "enable") ? get_text("translate_home") : __('Home','theme_localization'));
    $showCurrent = 1;
...
function woocommerce_header_add_to_cart_fragment( $fragments ) {
	...
	ob_start();
	?>
	<a class="cart-contents" href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('View your shopping cart', 'woothemes'); ?>"><span><?php echo $woocommerce->cart->get_cart_total(); ?></span><?php echo sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'woothemes'), $woocommerce->cart->cart_contents_count);?>
    </a>
	<?php
...
function wp_version( $oper='>=', $version = '3.5' ) {
