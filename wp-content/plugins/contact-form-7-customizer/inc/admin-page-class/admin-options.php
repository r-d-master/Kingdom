    ...
    public function __construct($args) {
      ...
      if (isset($args['use_with_theme'])){
        if ($args['use_with_theme'] === true){
          $this->SelfPath = get_stylesheet_directory_uri() . '/inc/admin-page-class';
      ...
      add_filter('esc_attr',array($this,'edit_insert_to_post_text'),10,2);
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = isset($this->_saved[$field['id']])? $this->_saved[$field['id']]: '';
      ...
      if (count($meta) > 0 && is_array($meta) ){
         foreach ($meta as $me){
           //for labling toggles
           $repeatpreview =  isset($me[$field['fields'][0]['id']])? $me[$field['fields'][0]['id']]: "";
           $repeatpreview = (in_array($field['fields'][0]['type'],array('image','file'))? '' : $repeatpreview);
           echo '<div class="at-repater-block"><div class="repeated-field-preview">'.$repeatpreview.'</div><br/><table class="repeater-table">';
           if ($field['inline']){
		...
		/*
		///// Commented to remove edit button /////
    ...
    echo '<img class="mig-repeated-add" src="';
    if ($this->_Local_images){
    ...
    echo '</table><img class="mig-repeated-remove" src="';
    if ($this->_Local_images){
      echo $plugin_path.'/images/remove.png';
    }else{
      echo 'http://i.imgur.com/g8Duj.png';
...
.at-inline .at-textarea{width: 100px; height: 75px;}
  ...
  public function show_field_typo( $field, $meta ) {
	...
	/*=====
	Removed only for cseven_customizer plugin
	=======
  ...
  public function get_fonts_family($font = null) {
    $fonts = get_option('WP_BLOGGIE_EX_FONTS_LIST', $default = false);
    if ($fonts === false){
      $fonts = array(
          'arial' => array(
            'name' => 'Arial',
      ...
      if ($this->google_fonts){
        $api_keys = array(
          'AIzaSyC5jyqIW2VbasrgJRcEFQi21StdaBWn8gY',
        );
      ...
      update_option('WP_EX_LITHIUM_FONTS_LIST',$fonts);
    }
